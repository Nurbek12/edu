      ...
      <v-col cols="12" md="5" class="d-flex justify-end align-start">
        <v-card width="100%" color="surface" class="sticky">
          ...
          <v-card-text class="mt-4">
            <div class="d-flex" style="flex-wrap: wrap; gap: 10px">
              <v-chip
                ...
                v-for="(q, i) in questions"
                :key="i"
                :color="q.selected ? 'primary' : 'white'"
                variant="elevated"
              >
                {{ i + 1 }}
              </v-chip>
          ...
          <v-card-actions class="pl-5">
            <span>{{ time }}</span>
            <v-spacer />
            <v-btn variant="flat" color="primary" @click="finish"
              >Yakunlash</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-col cols="12" md="5" class="d-flex justify-end align-start">
        <v-card width="100%" color="surface" class="sticky">
          ...
          <v-card-text class="py-1"
            >Test nomi: {{ test.test.name }}</v-card-text
          >
          <v-card-text class="py-1"
          ...
          <v-card-text class="py-1"
            >To'gri: {{ test.rate }} / {{ questions.length }}
          </v-card-text>
          <v-card-text class="py-1"
            >Foiz:
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn variant="flat" color="primary" @click="prev()"
              >Ortga qaytish</v-btn
            >
          </v-card-actions>
        </v-card>
    ...
    <v-row v-else>
      <v-col cols="12">
        <v-sheet elevation="1" color="surface" width="100%" class="pa-2" rounded>
          <div class="d-flex justify-center text-red">Ushbu Test Topilmadi!</div>
...
function start() {
  let t = Math.floor(
    test.value.exam.time * 60 -
      (Date.now() - test.value.start_time) / 1000
  );
...
const init = async () => {
  ...
  if (data?.status === "start") {
    socket.emit("push-data", {
      ...
      test: test.value.test?.name,
      result: test.value._id,
      device: (navigator.userAgent || "(Nomalum qurilma)").match(/\([^\)]+\)/)[0],
