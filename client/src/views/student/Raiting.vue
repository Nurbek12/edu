    ...
    <v-row>
      <v-col cols="12">
        <v-sheet elevation="1" color="surface" width="100%" class="pa-2" rounded>
          <div class="d-flex justify-center">YAKUNIY NATIJALAR</div>
        </v-sheet>
      </v-col>
      <v-col cols="12">
        <v-sheet
          ...
          <div class="responsive-datatable">
            <v-data-table items-per-page-text :headers="headers" :search="search" :items="items">
              <template #item.test="{ index, item }">
                <td data-label="Fan nomi">{{ index + 1 }}. {{ item.raw.test?.name }}</td>
              </template>
              ...
              <template #item.view="{ item }">
                <td data-label="Ko'rish">
                  <v-btn size="30" color="primary" variant="flat" :to="`/tests/${item.raw._id}`">
...
const initialize = async (distance) => {
  const { data } = await axios.get(`/test/result?user=${store.getters.userid}&${distance}`);
  items.value = data;
};
