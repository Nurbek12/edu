          ...
          <div class="responsive-datatable">
            <v-data-table no-data-text="Hech narsa topilmadi" items-per-page-text :headers="headers" :search="search" :items="items">
              <template #item.name="{ index, item }">
                <td data-label="Test Nomi">{{ index + 1 }}. {{ item.raw.name }}</td>
              </template>
              ...
              <template #item.time="{ item }">
                <td data-label="Muhlari">{{ item.raw.time }}</td>
              </template>
              ...
              <template #item.count="{ item }">
                <td data-label="Testlar soni">{{ item.raw.count }}</td>
              </template>
              ...
              <template v-slot:item.actions="{ item }">
                <td data-label="Boshqarish">
                  <div class="d-flex justify-end">
                    ...
                    <v-btn variant="flat" class="ml-1"
                      @click="viewItem(item.raw._id, item.raw.name)" size="30" color="primary"
                    >
              ...
              <v-row>
                ...
                <v-col cols="12" class="px-3">
                  <v-row>
                    ...
                    <v-col cols="12" v-if="editedId === -1">
                      <v-btn
                        variant="flat"
                        color="primary"
                        block @click="addQuestion"
                        height="45"
                      >
                        Savol Qo'shish
                      </v-btn>
    ...
    <v-dialog v-model="testdialog" max-width="550px" persistent>
      <v-card class="test-scroll">
        <v-card-title class="mt-2 ml-2 mb-0 text-h6 d-flex justify-space-between align-center">
          ...
          <v-spacer></v-spacer>
          <v-btn size="30" class="mr-2" color="primary" @click="testdialog=false">
              ...
              <v-row>
                <v-col cols="12">
                  <v-card variant="tonal" class="pa-2">
                    ...
                    >
                      <template v-slot:append-inner>
                        <v-btn size="30" variant="flat"
                          @click="validate1" color="primary">
...
import RePossible from '../../components/RePossibleStudents.vue'

const headers = [
  { title: "Test Nomi", key: "name" },
  { title: "Fan", key: "subject", sortable: false },
  { title: "Guruh", key: "group", sortable: false },
  { title: "Sanasi", key: "date", sortable: false },
  { title: "Muhlati", key: "time", sortable: false },
  { title: "Testlar soni", key: "count", sortable: false },
...
const formTitle = computed(() =>
  editedIndex.value === -1 ? "Oraliq Yaratish" : "Oraliq O'zgartirish"
);
...
const initialize = async () => {
    const [groupdata, subjectdata, testdata] = await Promise.all([
      axios.get("/group/access"),
      axios.get("/subject/access"),
      axios.get("/midterm"),
    ]);
    items.value = testdata.data;
    groups.value = groupdata.data;
    subjects.value = subjectdata.data;
  },
  validate = async () => {
    const { valid } = await form.value.validate();
    if (valid) save()
  },
  viewItem = async (id, name) => {
  ...
  editItemQs = async (item) => {
    if(!item.questions){
      const { data } = await axios.get(`/midterm/id/${item._id}`)
  ...
  deleteItemConfirm = async () => {
    const { data } = await axios.delete(`/midterm/${editedId.value}`);
    if (data) {
  ...
  save = async () => {
    if (editedIndex.value > -1) {
      const { data } = await axios.put(`/midterm/${editedId.value}`, editedItem.value);
      Object.assign(items.value[editedIndex.value], data);
    } else {
      const { data } = await axios.post("/midterm", {
  ...
  createQuestion = async () => {
    if(editedqsid.value !== null) {
      ...
      const { data } = await axios.post('/test/question', {midterm: editedId.value,...questionRefs.value})
